...
int MyHandler::pushData(Connector* client, httpRequest* httpReq)
{
    ...
    char* retBuf=NULL;
    int retBufLen = 0;
    if(packSuccessRes(retBuf, retBufLen, "req=pushdata", strlen("req=pushdata")) > 0){
...
int MyHandler::regUser(ServerClient* client, httpRequest* httpReq)
{
    ...
    char* retBuf=NULL;
    int retBufLen = 0;
    if(packSuccessRes(retBuf, retBufLen, "req=register&state=success", strlen("req=register&state=success")) > 0)
    {
        client->send_msg(retBuf, retBufLen);
        free(retBuf);
        client->close();  
