...
__ENTER_FUNCTION
	Assert(head);
	int copiedLength = 0;
	copiedLength = strUtil::Snprintf(_sql, MAX_SQL_LENGTH, "%s", head);
	if (-1 == copiedLength)
	{
		return false;
	}
...
__ENTER_FUNCTION
	...
	_curCharCount += strUtil::Snprintf(_sql + _curCharCount, MAX_SQL_LENGTH, "%s", str);
	return true;
__LEAVE_FUNCTION
	return false;
}
...
__ENTER_FUNCTION
	//TODO sprintf执行失败没有处理
	_curCharCount += strUtil::Snprintf(_sql + _curCharCount, MAX_SQL_LENGTH, "%d", param);
	return true;
__LEAVE_FUNCTION
	return false;
}
...
{
	__ENTER_FUNCTION
	return push(string, strUtil::Strlen(string) + 1);
	__LEAVE_FUNCTION
	return false;
}
