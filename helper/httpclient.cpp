...
int HttpClient::init(size_t res_buf_size, unsigned long cnn_timeout, unsigned long read_timeout)
{
    if (res_buf_size<=0)
    {
        return -1;
    ...
    {
        _mem_buf.memory = new char[res_buf_size];
        _mem_buf.size = res_buf_size;
        if (_mem_buf.memory == NULL)
            return -1;
    }
    _mem_buf.len = 0;
    ...
    //timeout
    curl_easy_setopt(_easy_handle, CURLOPT_CONNECTTIMEOUT, cnn_timeout);
    curl_easy_setopt(_easy_handle, CURLOPT_TIMEOUT, read_timeout);
...
int HttpClient::do_request(const char *url, void* binaryptr, int size)
{
    _mem_buf.len = 0;
    if (url == NULL)
    {
...
char* HttpClient::get_response(uint *status_code, size_t* response_len)
{
    if (status_code != NULL)
    {
