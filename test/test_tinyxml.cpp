...
int getChildrenCount(TiXmlNode* node)
{
    int count = 0;
...
bool checkChildren(TiXmlNode* node, vector<string>& childrenName)
{
    int count = 0;
    for (TiXmlNode* child = node->FirstChild(); child; child = child->NextSibling())
    {
...
TiXmlNode* getSuccessor(TiXmlNode* node, const char * val)
{
    ...
    for (TiXmlNode* child = node->FirstChild(); child; child = child->NextSibling())
    {
        TiXmlNode* cn = getSuccessor(child, val);
        if (cn)
        {
            return cn;
        }
...
{
	{
		...
		TiXmlDocument doc( "./testdata/demotest.xml" );
		doc.Parse( demoStart );
		if ( doc.Error() )
		{
	...
	{
		TiXmlDocument doc( "./data/demotest.xml" );
