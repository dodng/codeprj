...
int test_httpost()
{
	...
	//访问url，获取结果
	strUtil::Snprintf(url,1024,"http://10.48.57.53:8692/v?ct=301989888&rn=20&pn=0&db=0&s=25&ie=utf-8&mcpacktest=1&word=exo");
	int ret = obj.do_request(url,postdata,sizeof(postdata));
	if(ret == 0) 
	{
		size_t retLen = 0;
		uint code = 0;
		char *res = obj.get_response(&code, &retLen);
		if (code != 200) 
		{
...
int get_query_result(Table<TableRowAudit>& table, vector<string>& result)
{
	...
	for (int i=0; i<count; ++i) 
	{
		...
		strUtil::Snprintf(url,1024,"http://v.baidu.com/v?word=%s&ct=301989888&rn=20&pn=0&db=0&s=0&fbl=800&ie=utf-8&otype=json_out&ifrom=video",
			pRow->m_query);
		int ret = obj.do_request(url,NULL,0);
		if(ret == 0) {
			size_t retLen = 0;
			uint code = 0;
			char *res = obj.get_response(&code, &retLen);
			...
			for (int j=0; j<length; ++j)
			{
				...
				char info[64] = {0};
				strUtil::Snprintf(info,64,"\t\tid=%d\t\tty=%d\n", id,ty);
				field.append(info);
				result.push_back(field);
			}
		}
