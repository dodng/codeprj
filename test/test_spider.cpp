...
bool DBInterface::loadPersonData(const char* sql)
{
    ...
    strUtil::Snprintf(SQLString,MAX_SQL_LEN,sql,PERSON_TABLE);
    bool bResult = pDataBase->execute(SQLString,strUtil::Strlen(SQLString));
    Assert(bResult);
...
bool DBInterface::savePersonData(QTYPE qtype, std::list<stPersonResult>& _dataQ)
{
    ...
    for (; iter != _dataQ.end(); ++iter)
    {
        if (qtype == BK_Q)
        {
            strUtil::Snprintf(sqlString,sizeof(sqlString),UPDATE_PERSON_BK_SQL, PERSON_TABLE,
                iter->_bk_itemid, iter->_bk_subLemmaId, iter->_bk_new_subLemmaId, iter->_id);
        }
        else if (qtype == TP_Q)
        {
            strUtil::Snprintf(sqlString,sizeof(sqlString),UPDATE_PERSON_TP_SQL, PERSON_TABLE,
                iter->_tp_json.c_str(), iter->_id); 
            //discard sql
            if (strUtil::Strlen(sqlString) >= 1024*3)
            {
                LogSystem::getSinglePtr()->cacheLog(LOG_FILE_EXCEPTION,"Save json fail,person_id=%d", iter->_id);
                continue;
            }    
        }
        //strUtil::Strncat(tmpString,sqlString,strUtil::Strlen(sqlString));
	    ret = pDataBase->execute(sqlString, strUtil::Strlen(sqlString));
    }
    return ret;
}
...
bool DBInterface::saveRankData(vector<Person>& data)
{
    ...
    strUtil::Snprintf(tmpString,sizeof(tmpString),DEL_PERSON_RANK_SQL, RANK_TABLE);
    bool ret = pDataBase->execute(tmpString, strUtil::Strlen(tmpString));
    if (!ret)
    {
        return false;
    }
    memset(tmpString,0,sizeof(tmpString));
    strUtil::Snprintf(tmpString,sizeof(tmpString),INSERT_PERSON_RANK_SQL, RANK_TABLE);
    for (int i=0; i<data.size(); ++i)
    {
        strUtil::Snprintf(sqlString,sizeof(sqlString), "(%d, \'%s\', %d, \'%s\' ),", 
            data[i].rank, data[i].name.c_str(), data[i].searchCnt, data[i].image.c_str());
        strUtil::Strcat(tmpString, sqlString);
    }
    tmpString[strUtil::Strlen(tmpString)-1] = 0;
    ret = pDataBase->execute(tmpString, strUtil::Strlen(tmpString));
    return ret;
...
int SpiderMgr::_requestToken(const char* token_url, char* resp_buf, int resp_buf_len)
{
    ...
    if(_h.init(resp_buf_len,5,5) < 0)
        return false;
    strUtil::Snprintf(req_buf,sizeof(req_buf),token_url,_api_key.c_str(),_secret_key.c_str());
    int ret = _h.do_request(req_buf,NULL,0);
    if(ret !=0)
        return -1;

    size_t ret_len = 0;
    uint code = 0;
    char* res = _h.get_response(&code,&ret_len);
    if(code != 200 )
        return -1;
...
bool Spider::getTP(int id, string& from, vector<stTPJsonItem>& relations)
{
    ...
    for (int i=0; i<objCnt; ++i)
    {
        ...
        printf("fromId=%d\n", strUtil::Strtoint(fromIdVal));
        if (id != strUtil::Strtoint(fromIdVal))
        {
            continue;
        }
        /////////////////////////////////////////////////////////
        ...
        strUtil::Utf8ToGbk(fromNameVal, from);
        printf("fromName=%s\n", from.c_str());
        //from = fromNameVal;
        /////////////////////////////////////////////////////////
        ...
        for (int j=0; j<attrCnt; ++j)
        {
            ...
            //get relation
            if (strUtil::Strncmp(attrNameVal,"name", strUtil::Strlen(attrNameVal))==0)
            {
                ...
                strUtil::Utf8ToGbk(attrValueVal,child.relation);
                printf("relation=%s\n", child.relation.c_str());
                //child.relation = attrValueVal;
            }
            //get toId
            else if (strUtil::Strncmp(attrNameVal,"toId", strUtil::Strlen(attrNameVal))==0)
            {
                struct json_object* attrValue  = json_object_object_get(attr, "VALUE");
                if(NULL == attrValue || is_error(attrValue)){
                    json_object_put(attrs);
                ...
                const char* attrValueVal = json_object_get_string(attrValue);
                printf("toId=%s\n", attrValueVal);
                child.toId = strUtil::Strtoint(attrValueVal);
            }
            //get toName
            else if (strUtil::Strncmp(attrNameVal,"toName", strUtil::Strlen(attrNameVal))==0)
            {
                struct json_object* attrValue  = json_object_object_get(attr, "VALUE");
                if(NULL == attrValue || is_error(attrValue)){
                    json_object_put(attrs);
                ...
                strUtil::Utf8ToGbk(attrValueVal,child.toName);
                printf("toName=%s\n", child.toName.c_str());
            }
        }
...
{
    char req_buf[1024] = {0};
    strUtil::Snprintf(req_buf, sizeof(req_buf), card_url, SpiderMgr::getSinglePtr()->_access_token.c_str(),
...
{
    char req_buf[512] = {0};
    strUtil::Snprintf(req_buf, sizeof(req_buf), relation_url);
    char param_buf[512] = {0};
    strUtil::Snprintf(param_buf, sizeof(param_buf), "g.has('fromId',MATCH,'%d').limit(8).with('*')", id);
    ...
    //cout << jsonstr << endl;
    char postdata[512] = {0};
    memcpy(postdata,jsonstr,strlen(jsonstr));

    if(_h.init(resp_buf_len,5,5) < 0)
        return -1;

   int ret = _h.do_request(req_buf,postdata,strlen(postdata));
   if(ret !=0)
        return -1;
...
void Spider::dwloadBKMultiPerson(int index)
{
    ...
    for (int i = start; i < end; ++i)
    {
        ...
        strUtil::Snprintf(buff, sizeof(buff), "videoId=%d, name=%s, bkId=", 
               (*_persons)[i]._id, (*_persons)[i]._name_chs.c_str());

        char temp[512] = {0};
        for (int j=0; j<list.size(); ++j)
        {
            strUtil::Snprintf(temp,sizeof(temp), "(%d,%d),", list[j], newlist[j]);
            strUtil::Strcat(buff, temp);
        }
...
void Spider::dwloadBK(int index)
{
    ...
    for (int i = start; i < end; ++i)
    {
        ...
        {
            LogSystem::getSinglePtr()->cacheLog(LOG_FILE_EXCEPTION,"get bk_id error,bk_key=%s", 
                (*_persons)[i]._name_chs.c_str());
            continue;
        }
...
void Spider::dwloadTP(int index)
{
    ...
    for (int i = start; i < end; ++i)
    {
        ...
        {
            LogSystem::getSinglePtr()->cacheLog(LOG_FILE_EXCEPTION,"get tupu error,bk_id=%d", 
                (*_persons)[i]._newSubLemmaId);
            continue;
        }
        //generate json
        string json;
...
bool Spider::genJson(int fromId, string& fromName, vector<stTPJsonItem>& relations, string& json)
{
    ...
    if (root)
    {
        ...
        json = jsonstr;
        //todo: 转成gbk
        return true;//strUtil::Utf8ToGbk(jsonstr, json);
    }
    return false;
}
...
bool test_bk()
{
    ...
    if(!ret)
    {
        LogSystem::getSinglePtr()->saveLogImmediately(LOG_FILE_ERROR,"get token error.");
        return false;
    }
    ...
    if (!ret)
    {
        LogSystem::getSinglePtr()->saveLogImmediately(LOG_FILE_ERROR,"init DBinterface fail.");
        return false;
    }
    ...
    {
        LogSystem::getSinglePtr()->saveLogImmediately(LOG_FILE_ERROR,"get persons from db error.");
        return false;
    }
    ...
    test._id = 6543;
    //test._name_chs = "郑爽";
    strUtil::GbkToUtf8("郑爽", test._name_chs);
    persons->clear();
    ...
    pSpiderMgr->startBK();
    pSpiderMgr->waitBK();
    LogSystem::getSinglePtr()->saveLogImmediately(LOG_FILE_ERROR,"qsize=%d", 
    ...
    for (TiXmlNode* child = node->FirstChild(); child; child = child->NextSibling())
    {
        ...
        if (name=="class" && value=="first")
        {
            ...
            if (cn && cn->FirstChild())
            {
                ...
                if (ct)
                {
                    rank = strUtil::Strtoint(ct->Value());
                    rankFlag = true;
                }
            }
        }
        else if (name=="class" && value=="keyword")
        {
            ...
            if (cn && cn->FirstChild())
            {
                ...
                if (ct)
                {
                    //strUtil::Utf8ToGbk(ct->Value(), personName);
		    personName = ct->Value();
                    personFlag = true;
                }
            }
        ...
        else if (name=="class" && value=="last")
        {
            ...
            if (cn && cn->FirstChild())
            {
                ...
                if (ct)
                {
                    searchCnt = strUtil::Strtoint(ct->Value());
                    searchFlag = true;
                }
            }
        }
...
bool test_spider()
{
    ...
    if(!ret)
    {
        LogSystem::getSinglePtr()->saveLogImmediately(LOG_FILE_ERROR,"SpiderMgr init error.");
        return false;
    }
